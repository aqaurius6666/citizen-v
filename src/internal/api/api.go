...
func (s *ApiServer) RegisterEndpoint() {
	...
	citizen.POST("/export", s.AuthMiddleware.CheckAuth, s.Citizen.HandlePostExport)
	...
	campaign.POST("/:id/close", s.AuthMiddleware.CheckAuth, s.RoleMiddleware.OnlyActive(), s.RoleMiddleware.OnlyRole(role.ROLE_A2, role.ROLE_A3, role.ROLE_B1, role.ROLE_B2), s.Campaign.HandlePostClose)
