...
//go:generate go run main/main.go api
func GenCode() {

}

var swaggerList map[string]SwaggerStruct = map[string]SwaggerStruct{
	"api": {JsonDoc: API_JSON},
}

func CustomWrapHandler(config *ginSwagger.Config, h *webdav.Handler) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if path == ".json" {
			...
			path = p[len(p)-1] + path
			prefix = strings.Join(p[1:len(p)-1], "/")
		}
		locker.Lock()
		h.Prefix = prefix
		locker.Unlock()
		...
		if strings.HasSuffix(path, ".json") {
			name := strings.Split(path, ".json")[0]
			println("name: ", name)
			jsonObj, ok := swaggerList[name]
			if !ok {
